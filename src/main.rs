...
use octocrab::{
    self, Octocrab,
    params::{Direction, State, issues::Sort, pulls},
};
...
const COMPLETE: owo_colors::Style = OwoStyle::new().green().bold();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.pr_history {
        for i in 0..=50 {
            if let Ok(()) =
                pr_history(Box::new(octo.clone()), Box::new(i), args.pr_history_read).await
            {
                return Ok(());
            }
        }
    ...
    for item in page.items {
        if Path::new(&format!("issues_repros/id{}.rs", item.number)).exists()
            || Path::new(&format!("issues_repros/triaged/id{}.rs", item.number)).exists()
        {
            continue;
        }
    ...
    for id in to_review {
        ...
        dbg!(
            &Path::new(&std::env::current_dir().unwrap())
                .join("issues_repros")
                .join(format!("id{}.rs", id.to_string()))
                .display()
                .to_string()
        );
        ...
        let output = Command::new("clippy-driver")
            ...
            .output()
            .expect("Failed to start the cargo command");
        if let Some(code) = output.status.code()
            && code != 1
        {
            // Other errors are 130 and 131
...
fn only_test_repro(ps: &SyntaxSet, ts: &ThemeSet) {
    ...
    for path in paths {
            ...
            && code != 1
        {
            // Other errors are 130 and 131
            println!(
                "{} Couldn't be reproduced, what happened? Checkout <https://github.com/rust-lang/rust-clippy/issues/{}>",
                &path.file_name().into_string().unwrap()[2..][..5],
                &path.file_name().into_string().unwrap()[2..][..5]
            );
            ...
            if !s.contains("clippy") {
                println!(
                    "Something's wrong with {}? An error happens but not with Clippy",
                    &path.file_name().into_string().unwrap()[2..][..5]
                );
            }
            // dbg!(&s);
...
fn profile(
    ...
    for cap in re.captures_iter(s) {
        ...
        master_ir_collected += cap[0].split("Collected : ").collect::<Vec<&str>>()[1]
            .parse::<i128>()
            .unwrap();
    }
    ...
    for cap in re.captures_iter(s) {
        dbg!(cap[0].split("Collected : ").collect::<Vec<&str>>());
        branch_ir_collected += cap[0].split("Collected : ").collect::<Vec<&str>>()[1]
            .parse::<i128>()
            .unwrap();
    }
    ...
    let result = ((master_ir_collected as f64 - branch_ir_collected as f64)
        / master_ir_collected as f64)
        * 100.0f64;
    if result.abs() >= 0.19f64 {
        println!(
            "{}% {}",
            ((master_ir_collected as f64 - branch_ir_collected as f64)
                / master_ir_collected as f64)
                * 100.0f64,
            if master_ir_collected > branch_ir_collected {
...
async fn pr_history(octo: Box<Octocrab>, page: Box<u32>, read: bool) -> Result<(), ()> {
    ...
    for pull in pulls {
        ...
        println!(
            "{} - {}",
            pull.url.split("/").last().unwrap(),
            pull.user.unwrap().login
        );
